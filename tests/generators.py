...
def generate_random_palindrome(min_len: int = 20, max_len: int = 25) -> str:
    ...
    half_palindrome = "".join(
        random.choices("abcdefghijklmnopqrstuvwxyz", k=half_palindrome_length)
    )
...
def generate_test_phrase() -> tuple[str, str]:
    ...
    base_text = fake.text(max_nb_chars=max_phrase_len * 2)[
        : max_phrase_len - len(palindrome)
    ]

    normalized_text = normalize(base_text)
    insert_position = random.randint(0, len(base_text))
    phrase = (
        normalized_text[:insert_position]
        + palindrome
        + normalized_text[insert_position:]
    )
